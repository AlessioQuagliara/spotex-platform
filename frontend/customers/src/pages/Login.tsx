  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.message === 'Conferma prima la tua email') {
          setMessage('❌ Devi prima verificare la tua email. Controlla la tua casella di posta.');
        } else {
        ...
        <div className="bg-white py-8 px-6 shadow-2xl rounded-2xl border border-gray-200 transform hover:shadow-3xl transition-all duration-300">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <input
                  ...
                  onChange={handleChange}
                  className="h-4 w-4 rounded border-gray-300 focus:ring-blue-500 transition duration-200"
                  style={{ color: primaryColor }}
                />
            ...
            <div className="relative">
              ...
              >
                <i className={`fas ${isLoading ? 'fa-spinner animate-spin' : 'fa-sign-in-alt'} mr-2`}></i>
                {isLoading ? 'Accesso in corso...' : 'Accedi al tuo account'}
              </button>
            </div>
          </form>
